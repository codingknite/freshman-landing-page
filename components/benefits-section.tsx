              ...
              {additionalItems.map((item: any, idx: number) => {
                ...
                return (
                  <div key={idx} className='flex flex-col items-center text-center'>
                    <div className='w-12 h-12 rounded-full bg-indigo-500 flex items-center justify-center mb-4 shadow-sm'>
...
function BenefitCard({ benefit, isActive, onClick }: { benefit: BenefitFeature, isActive: boolean, onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative flex flex-col text-left p-6 md:p-8 rounded-[2rem] transition-all duration-300 ease-out border w-full cursor-pointer group ${
        isActive 
    ...
    >
      <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center mb-5 md:mb-6 transition-all duration-300 ${
        isActive 
