    ...
    <section className='bg-[oklch(0.992_0.004_260/0.96)] py-16 md:py-22 lg:py-24 relative overflow-hidden'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10'>
        <div className='mx-auto mb-16 max-w-3xl text-center md:mb-24 lg:mb-32'>
          ...
          <h2 className='text-balance text-2xl font-medium tracking-tight text-slate-900 sm:text-4xl lg:text-5xl'>
            {t('features.title')}
          </h2>
        </div>
        ...
        <div className='mt-24 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 lg:gap-20'>
          {localizedItems.map(
            (item: { title: string; description: string }, index: number) => (
              <article
                key={index}
                className='flex flex-col items-center text-center max-w-[320px] mx-auto'
              >
                ...
                <p className='!text-[15.5px] leading-relaxed text-slate-500 sm:text-base'>
                  {item.description}
                </p>
              </article>
            ),
